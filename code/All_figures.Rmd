...
pheatmap(class_average_correlations, 
...
pheatmap(
  class_average_correlations, 
         ...
         border_color = NA,
         angle_col = "315",
  filename = here(output_dir, "Figure4.png"),
  width = 8,
  height = 5
)
...
pheatmap(
  class_average_correlations, 
         ...
         border_color = NA,
         angle_col = "315",
  filename = here(output_dir, "Figure4.pdf"),
  width = 8,
  height = 5
)
...
pheatmap(
  class_average_correlations, 
         ...
         border_color = NA,
         angle_col = "315",
  filename = here(output_dir, "Figure4.tiff"),
  width = 8,
  height = 5
)
```
...
all_plots_5 <- plot_grid(
  plot_grid(PE_TG_correlation_rawdata,
            PE_TG_correlation_merged, PE_TG_correlation_by_structural_lipids,
            #PE_no_TG + labs(y = "PE"),
            labels = LETTERS[1:3], nrow = 1),
  plot_grid(TG_merged + labs(y = "TG"), 
            PE_merged + labs(y = "PE"), 
            TG_by_structural_lipids + labs(y = "TG"), 
            PE_by_structural_lipids + labs(y = "PE"),
            labels = LETTERS[4:7], nrow = 1),
  align = 'v',
  hjust = -0.5,
  nrow = 2
)
...
make_comparison_plot <- function(PCA_comparison, x, y, multiplyPC){
  p <- PCA_comparison %>%
    ...
    geom_abline(slope = 1*multiplyPC, color = "grey")
  return(p)
}
...
```{r}
# Deviating replicate in TG mode
comparison_deviating_replicate_df <- read_csv(here(data_dir, "comparison_deviating_replicate_df.csv"))
head(comparison_deviating_replicate_df)
table(comparison_deviating_replicate_df$is_triglyceride)
```

A figure showing that one replicate deviates in TG mode for the treatment study.

```{r}
plot_tg <- function(col_name, replicate_x, replicate_y){
  df <- comparison_deviating_replicate_df %>%
    dplyr::select(biological_replicate, lipid, all_of(col_name), class, is_triglyceride) %>%
    pivot_wider(names_from = biological_replicate, values_from = col_name)
  
  p <- df %>%
    ggplot(aes(x = !!as.name(replicate_x), y = !!as.name(replicate_y), color = is_triglyceride)) +
    geom_point() +
    personal_theme() +
    ...
    scale_color_manual(values = color_legend_triglycerides)
  return(p)
}
```
...
all_plots_sup1 <- plot_grid(
  plot_tg("log2_abundance", "replicate 1", "replicate 2"),
  plot_tg("log2_abundance", "replicate 1", "replicate 3"),
  plot_tg("log2_abundance", "replicate 2", "replicate 3"),
...
save_plot(
  here(output_dir, "SupplementaryFigure1_legend.png"),
  the_legend_sup1)
```
