...
This Rmd script reproduces all calculations performed for the manuscript "Normalization strategies for lipidome data in cell line panels". The data cannot be published with this manuscript yet, because we are still interpreting the biological processes. Metabolomics data have been deposited to the EMBL-EBI MetaboLights database (DOI: 10.1093/nar/gkad1045, <PMID:37971328>) with the identifier MTBLS9493 and is accessible at <https://www.ebi.ac.uk/metabolights/MTBLS9493>.
...
All data is in a data subfolder:
...
lipid_info <- read_csv(here(data_dir, "lipid_info.csv")) %>%
  filter(class != "PG_or_BMP") # removed because we cannot distinguish between the two classes

print(dim(lipid_info))
head(lipid_info)
unique(lipid_info$class)
write_csv(lipid_info, here(output_dir, "lipid_info.csv"))
```

## Figure 1

Figure 1. Variation in data is mainly influenced by the median signal in a sample.
...
raw_data <- read_csv(here(data_dir, "raw_data.csv")) %>%
  dplyr::select(Original_name, all_of(lipid_info$internal_identifiers))
...
Figure 3. Data-based normalizations improve similarity between replicates.
...
Figure 4. Correlation between lipid classes before normalization shows an overall agreement 704 for structural lipids, but not for triglycerides and some signaling lipids.
...
Figure 5 Differences in PE class average between morphologies are partially created by normalization.
...
save_normalized_df(data_cell_lines_list[["PQN_SLNormalized_log2Transformed_notScaled"]], output_normalizations, "SL", "cell_lines", "PQNNormalized_log2Transformed_notScaled")
```
...
For the reviewer: what happens without autoscaling

```{r}
noAutoscale_class_averages_TGPE_rawdata <- data_cell_lines_list[["NotNormalized_log2Transformed_notScaled"]] %>%
  calculate_lipid_class_averages()
noAutoscale_class_averages_merged <- data_cell_lines_list[["PQNNormalized_log2Transformed_notScaled"]] %>%
  calculate_lipid_class_averages()
noAutoscale_class_averages_by_structural_lipids <- data_cell_lines_list[["PQN_SLNormalized_log2Transformed_notScaled"]] %>%
  calculate_lipid_class_averages()

calculate_TG_PE_cor(noAutoscale_class_averages_TGPE_rawdata)
calculate_TG_PE_cor(noAutoscale_class_averages_merged)
calculate_TG_PE_cor(noAutoscale_class_averages_by_structural_lipids)
```
...
write_csv(noAutoscale_class_averages_TGPE_rawdata, here(output_dir, "noAutoscale_class_averages_TGPE_rawdata.csv"))
write_csv(noAutoscale_class_averages_merged, here(output_dir, "noAutoscale_class_averages_merged.csv"))
write_csv(noAutoscale_class_averages_by_structural_lipids, here(output_dir, "noAutoscale_class_averages_by_structural_lipids.csv"))

```

## Figure 6

Figure 6 Correlation of principal components after data transformation shows similar results.
